...
const Dropdown = ({
  ...
  const teachPrompts = promptOptions.filter(
    (prompt) => prompt.category === "teach"
  );

  const otherPrompts = promptOptions.filter(
    (prompt) => prompt.category === "other"
  );
          ...
          <DropdownMenu.Sub>
            ...
            <DropdownMenu.Portal>
              ...
              >
                ...
                {teachPrompts.map((prompt, i) => (
                  <DropdownMenu.CheckboxItem
                    key={i}
                    className={styles.DropdownMenuItem}
                    checked={prompt.prompt === selectedPrompt.current[0]}
                    onSelect={() => {
                      abandonSession();
                      selectedPrompt.current[0] = prompt.prompt;
                      selectedPrompt.current[1] = prompt.asst;
